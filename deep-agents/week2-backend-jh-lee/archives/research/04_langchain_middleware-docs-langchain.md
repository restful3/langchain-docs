---
title: LangChain Middleware — Overview (control & customize agent execution)
url: https://docs.langchain.com/oss/python/langchain/middleware
fetched: 2026-05-15
source_type: 1차 (official LangChain docs)
fetch_method: curl + Mintlify .md endpoint
mapping: 교안 §8 Policy hooks — 미들웨어 기반 통제 컨텍스트
note: 본문 원문 그대로. 본 페이지는 PIIRedactionMiddleware, ModelCallLimit, HumanInTheLoop 등
      미들웨어를 통한 정책 제어를 정의. 백엔드 레벨의 GuardedBackend (서브클래싱) 와 함께
      §8 의 두 레이어 (백엔드 정책 + 미들웨어 정책) 비교에 사용.
---
