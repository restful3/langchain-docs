---
title: Ollama — Tool calling (function calling capabilities)
url: https://docs.ollama.com/capabilities/tool-calling
fetched: 2026-05-15
source_type: 1차 (official Ollama docs)
fetch_method: curl + Mintlify .md endpoint
mapping: 보조 — walkthrough.ipynb 의 ChatOllama(gemma4:31b) 도구 호출 안정성 검증 참고
note: 본문 원문 그대로. gemma4 모델별 tool calling 신뢰도 직접 언급은 없으나, Ollama 도구 호출
      API 의 메시지 포맷·streaming·tool 정의 컨벤션이 정리되어 있어 데모 실행 사전 준비에 필요.
      별도로 사용자 환경에서 'ollama show gemma4:31b' 로 tool-use 지원 여부 확인 권장.
---
