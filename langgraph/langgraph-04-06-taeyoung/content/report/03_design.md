...
<small>Figure — 고객지원 이메일 에이전트 그래프. **실선** 은 늘 같은 다음 노드로 가는 고정 엣지, **보라 점선** 은 노드 내부에서 `Command(goto=...)` 로 결정하는 분기다. (`human_review` 가 두 번 보이는 것은 같은 노드를 두 진입 경로 — 분류 직후 에스컬레이션, 초안 후 검토 — 로 나눠 그린 레이아웃 단순화다.)</small>
...
<small>Figure — 네 가지 스텝 유형과 이메일 에이전트에서의 예시 노드.</small>
