...
Step 1~4 를 합친 완성본 그래프(부모 폴더 email_agent.py)를 그대로 가져와 쓴다.
즉 이 파일은 '완성 그래프에 사람 개입을 붙여 돌리는' 마지막 단계다.
...
# 부모 폴더(scripts/)의 email_agent.py 를 import 할 수 있게 경로 추가
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from langgraph.checkpoint.memory import MemorySaver
from langgraph.types import Command

from email_agent import build_graph
...
if __name__ == "__main__":
    ...
    # 긴급 청구 → human_review 에서 멈춘다
    initial = {"email_content": "구독료가 두 번 청구됐어요! 급해요!",
               "sender_email": "customer@example.com", "email_id": "email_42"}
    config = {"configurable": {"thread_id": "customer_42"}}

    result = app.invoke(initial, config)
    print("\n  1) 첫 호출 → human_review 에서 일시정지")
    print(f"     interrupt payload: urgency={result['__interrupt__'][0].value['urgency']}")
    ...
    resume = Command(resume={"approved": True,
                             "edited_response": "이중 청구 사과드립니다. 즉시 환불 처리했습니다."})
    final = app.invoke(resume, config)
    print("\n  3) 사람 입력으로 재개 → 멈춘 지점부터 이어짐")
